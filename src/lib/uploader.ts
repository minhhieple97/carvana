  ...
  upload(file: File, part: FilePart, sendChunkStarted: () => void) {
    ...
    return new Promise((resolve, reject) => {
      if (this.fileId && this.fileKey) {
        ...
        const progressListener = this.handleUploadProgress.bind(this, part.PartNumber - 1);
  ...
  async sendCompleteRequest() {
    try {
      if (this.fileId && this.fileKey) {
        ...
        const result = await api.post(endpoints.images.finaliseMultipartUpload, {
      ...
      console.error('Complete request issue:', error);
    }
  }
