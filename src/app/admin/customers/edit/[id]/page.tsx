      ...
      <div className="container p-4 space-y-6">
        <div className="grid grid-cols-2 space-x-6">
          ...
          <Card className="bg-gray-800 border-gray-700">
            ...
            <CardContent className="grid gap-2 text-muted">
              ...
              <div>
                <strong>Terms Accepted:</strong> {customer.termsAccepted ? 'Yes' : 'No'}
              </div>
              <div>
        ...
        <Card className="bg-gray-800 border-gray-700">
          ...
          <CardContent className="grid gap-2 text-muted">
            ...
            <div>
              <strong>Created:</strong> {format(customer.createdAt, 'do MMM yyy HH:mm')}
            </div>
            <div>
              <strong>Last Updated:</strong> {format(customer.updatedAt, 'do MMM yyy HH:mm')}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-gray-800 border-gray-700">
          ...
          <CardContent className="grid gap-2 text-muted">
            <Table>
              <TableHeader>
                <TableRow>
                  <TableHead className="text-muted font-semibold text-base">Date</TableHead>
                  <TableHead className="text-muted font-semibold text-base">Old Status</TableHead>
                  <TableHead className="text-muted font-semibold text-base">New Status</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {customer.lifecycle.map((entry) => (
                  <TableRow className="text-muted/75" key={entry.id}>
                    <TableCell>{format(entry.updatedAt, 'do MMM yyy HH:mm')}</TableCell>
                    <TableCell>{formatCustomerStatus(entry.oldStatus)}</TableCell>
                    <TableCell>{formatCustomerStatus(entry.newStatus)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
